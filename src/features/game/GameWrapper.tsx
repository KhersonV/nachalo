...
export default function GameWrapper({
  ...
  async function fetchAll() {
    ...
    try {
      ...
      const [resources, monsters] = await Promise.all([
        fetch("http://localhost:8001/api/resources").then((r) => r.json()),
        fetch("http://localhost:8001/api/monsters").then((r) => r.json()),
      ]);
      ...
      console.error("Ошибка загрузки данных матча, ресурсов или монстров", e);
      router.replace("/mode");
    } finally {
  ...
  useEffect(() => {
    ...
    if (prevInstanceIdRef.current && prevInstanceIdRef.current !== instanceId) {
      console.log("[GameWrapper/useEffect] Новый instanceId, resetState()");
      dispatch(resetState());
    }
  ...
  const socket = useGameSocket(
    (data) => {
      if (data.payload?.instanceId !== instanceId) return;
