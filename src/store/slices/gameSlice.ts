...
const gameSlice = createSlice({
  ...
  if (action.payload.grid !== undefined) state.grid = action.payload.grid;
  if (action.payload.mapWidth !== undefined) state.mapWidth = action.payload.mapWidth;
  if (action.payload.mapHeight !== undefined) state.mapHeight = action.payload.mapHeight;
  if (action.payload.players !== undefined) state.players = action.payload.players;
  if (action.payload.active_user !== undefined) state.active_user = action.payload.active_user;
  if (action.payload.turnNumber !== undefined) state.turnNumber = action.payload.turnNumber;
  ...
  if (state.grid.length > 0 && state.mapWidth && state.mapHeight) {
   ...
   console.log("[gameSlice/setMatchData] new state:", {
    ...
    movePlayer(
      ...
      const p = state.players.find((p) => p.user_id === action.payload.userId);
    ...
    updatePlayer(
      ...
      const i = state.players.findIndex((p) => p.user_id === updated.user_id);
