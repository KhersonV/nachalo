...
func avatarForCharacterType(characterType string) string {
	switch strings.ToLower(strings.TrimSpace(characterType)) {
	case "guardian":
		return "/guardian/guardian.webp"
	case "berserker":
		return "/berserk/berserk.webp"
	case "ranger":
...
func buildPlayerGameStats(instanceID string, userID int, playerName string, results *game.MatchResults) (*playerGameStatsResult, error) {
	...
	return &playerGameStatsResult{
		...
		Player: playerGameStats{
			...
			Name:   playerName,
			Avatar: func() string {
				// prefer match player image, then persistent image, then archetype image based on character type
				if mp, merr := repository.GetMatchPlayerByID(instanceID, userID); merr == nil && mp != nil {
					if mp.Image != "" {
						return mp.Image
					}
					if mp.CharacterType != "" {
						return avatarForCharacterType(mp.CharacterType)
					}
				}
				if p, err := repository.GetPlayerByUserID(userID); err == nil && p != nil {
					...
					if p.CharacterType != "" {
						return avatarForCharacterType(p.CharacterType)
					}
				}
				return avatarForCharacterType("")
			}(),
...
func buildAllPlayersGameStats(instanceID string, results *game.MatchResults) ([]playerGameStats, string, int, error) {
	...
	for _, pr := range results.PlayerResults {
		...
		// choose avatar: prefer match player image, then persistent image, then archetype default
		avatar := avatarForCharacterType("")
		if mp, merr := repository.GetMatchPlayerByID(instanceID, pr.UserID); merr == nil && mp != nil {
			if mp.Image != "" {
				avatar = mp.Image
			} else if mp.CharacterType != "" {
				avatar = avatarForCharacterType(mp.CharacterType)
			}
		} else if p, perr := repository.GetPlayerByUserID(pr.UserID); perr == nil && p != nil {
			if p.Image != "" {
				avatar = p.Image
			} else if p.CharacterType != "" {
				avatar = avatarForCharacterType(p.CharacterType)
			}
		}
...
func buildAllPlayersGameStatsFromStored(instanceID string) ([]playerGameStats, string, int, error) {
	...
	for _, pr := range stored {
		...
		if playerName == fmt.Sprintf("Player %d", pr.UserID) {
			if p, perr := repository.GetPlayerByUserID(pr.UserID); perr == nil && p != nil && p.Name != "" {
				playerName = p.Name
			}
		}
		// avatar: prefer stored character_type -> archetype image, but use persistent image when set
		avatar := avatarForCharacterType("")
		if pr.CharacterType != "" {
			avatar = avatarForCharacterType(pr.CharacterType)
		}
		if p, perr := repository.GetPlayerByUserID(pr.UserID); perr == nil && p != nil {
			if p.Image != "" {
				avatar = p.Image
			}
		}
