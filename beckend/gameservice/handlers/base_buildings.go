...
type forgeStateResponse struct {
	...
	Tavern     buildingStateResponse `json:"tavern"`
}
...
func buildForgeState(userID int) (*forgeStateResponse, error) {
	...
	tavernCanBuild := !tavernBuilt &&
		woodCount >= repository.TavernCostWood &&
		stoneCount >= repository.TavernCostStone &&
		ironCount >= repository.TavernCostIron
	...
	return &forgeStateResponse{
		...
		Tavern: buildingStateResponse{
			Level:       tavernLevel,
			Built:       tavernBuilt,
			Costs:       forgeCostResponse{Wood: repository.TavernCostWood, Stone: repository.TavernCostStone, Iron: repository.TavernCostIron},
			Resources:   forgeResourceResponse{Wood: woodCount, Stone: stoneCount, Iron: ironCount},
			CanBuild:    tavernCanBuild,
			Unlockables: []forgeRecipeResponse{},
		},
	}, nil
}
...
func BuildTavernHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "Method Not Allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if err := repository.BuildTavern(userID); err != nil {
		if err.Error() == "tavern already built" {
			http.Error(w, `{"error":"tavern_already_built"}`, http.StatusConflict)
			return
		}
		if err.Error() == "not enough resources" {
			http.Error(w, `{"error":"not_enough_resources"}`, http.StatusBadRequest)
		...
		http.Error(w, `{"error":"failed_to_build_tavern"}`, http.StatusInternalServerError)
		return
	}
