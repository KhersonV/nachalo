...
func PlayerExists(userID int) (bool, error) {
	var exists bool
	if err := DB.QueryRow(`SELECT EXISTS(SELECT 1 FROM player_profiles WHERE user_id = $1)`, userID).Scan(&exists); err != nil {
...
func ListIncomingFriendRequests(userID int) ([]FriendRequestSummary, error) {
	rows, err := DB.Query(`
        SELECT
			p.user_id,
			p.name,
			COALESCE(pc.image, p.image),
			COALESCE(pc.hero_class_id, 'guardian'),
			COALESCE(pc.level, 1),
			pfr.created_at::text
        FROM player_friend_requests pfr
        JOIN player_profiles p ON p.user_id = pfr.requester_user_id
...
func ListOutgoingFriendRequests(userID int) ([]FriendRequestSummary, error) {
	rows, err := DB.Query(`
        SELECT
			...
			pfr.created_at::text
        FROM player_friend_requests pfr
        JOIN player_profiles p ON p.user_id = pfr.target_user_id
		LEFT JOIN player_characters pc ON pc.id = p.selected_character_id
...
func ListFriends(userID int) ([]FriendSummary, error) {
	rows, err := DB.Query(`
        ...
        JOIN player_profiles p ON p.user_id = pf.friend_user_id
		LEFT JOIN player_characters pc ON pc.id = p.selected_character_id
        WHERE pf.user_id = $1
        ORDER BY p.name ASC
    `, userID)
