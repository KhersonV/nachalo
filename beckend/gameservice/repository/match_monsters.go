...
func GetMatchMonsterAt(instanceID string, x, y int) (*MatchMonster, error) {
	...
	err := DB.QueryRow(`
        SELECT
          instance_id, monster_instance_id, monster_ref_id,
          pos_x, pos_y, health, max_health,
          attack, defense, speed, maneuverability, vision, image
        FROM match_monsters
        WHERE instance_id=$1 AND pos_x=$2 AND pos_y=$3
    `, instanceID, x, y).Scan(
...
func UpdateMatchMonsterHealth(instanceID string, monsterInstanceID, newHP int) error {
	_, err := DB.Exec(`
        UPDATE match_monsters
           SET health = $1
         WHERE instance_id = $2 AND monster_instance_id = $3
...
func GetMatchMonsterByID(instanceID string, monsterInstanceID int) (*MatchMonster, error) {
	var m MatchMonster
	err := DB.QueryRow(`
        SELECT instance_id, monster_instance_id, monster_ref_id,
               pos_x, pos_y, health, max_health,
               attack, defense, speed, maneuverability, vision, image
        FROM match_monsters
        WHERE instance_id=$1 AND monster_instance_id=$2
