...
func EnsureEquipmentSeedData() {
	if _, err := DB.Exec(`
		...
		VALUES (
			...
			'mystic',
			'green',
			'A beginner mystic set focused on energy, Reflex and early spell tempo.'
		)
		...
		WITH sagecloth AS (
			SELECT id AS set_id FROM item_sets WHERE code = 'sagecloth_set'
		),
		items AS (
			SELECT *
			FROM (VALUES
		...
		SELECT
			...
			items.handedness,
			items.rarity,
			items.class_restriction,
			items.image_url,
			items.attack_bonus,
			items.defense_bonus,
			items.mobility_bonus,
			items.agility_bonus,
			items.max_health_bonus,
			items.max_energy_bonus,
			items.sight_range_bonus,
			items.attack_range_bonus
		FROM items
		CROSS JOIN sagecloth
		ON CONFLICT (code) DO UPDATE SET
			name = EXCLUDED.name,
		...
		bonuses AS (
			SELECT *
			FROM (VALUES
				(2, '+5 Max Energy while wearing 2 pieces of Sagecloth Set.', 0, 0, 0, 0, 0, 5, 0, 0, NULL::text, 0),
				(4, '+1 Reflex while wearing 4 pieces of Sagecloth Set.', 0, 0, 0, 1, 0, 0, 0, 0, NULL::text, 0),
				(6, '+1 Attack while wearing 6 pieces of Sagecloth Set.', 1, 0, 0, 0, 0, 0, 0, 0, NULL::text, 0)
			) AS v(pieces_required, description, attack_bonus, defense_bonus, mobility_bonus, agility_bonus, max_health_bonus, max_energy_bonus, sight_range_bonus, attack_range_bonus, effect_code, effect_value)
		)
		...
		SELECT
			sagecloth.set_id,
			bonuses.pieces_required,
			bonuses.description,
			bonuses.attack_bonus,
			bonuses.defense_bonus,
			bonuses.mobility_bonus,
			bonuses.agility_bonus,
			bonuses.max_health_bonus,
			bonuses.max_energy_bonus,
			bonuses.sight_range_bonus,
			bonuses.attack_range_bonus,
			bonuses.effect_code,
